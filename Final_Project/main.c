...

{
    ...
    char hr_string_print[2], min_string_print[2], sec_string_print[2], colon_print[1] = ':', zero_print[1] = '0';
    char hr_string_alarm[2], min_string_alarm[2];
    ...
    INPUT_BUFFER[0] = '\0'; //initial condition of empty
...
//  Timer_32_init();
    ...
    while(1)
    {
        ...
        if((string[0] == 'R') && (string[4] == 'T'))
        {
            if(flag_time == 1)
            {
                now.hour = now.hour + 0b1100;
            }

            sprintf(hr_string_print, "%d", now.hour);
            sprintf(min_string_print, "%d", now.min);
            sprintf(sec_string_print, "%d", now.sec);

            if(now.hour < 10)
            {
                writeOutput(zero_print);
        ...
        if((string[0] == 'R') && (string[4] == 'A'))
        {
            ...
            sprintf(hr_string_alarm, "%d", hr_alarm);
            sprintf(min_string_alarm, "%d", min_alarm);

            if(hr_alarm < 10)
            {
                writeOutput(zero_print);
                writeOutput(hr_string_alarm);
                writeOutput(colon_print);
            }
            else
            {
...
/*
* void setupSerial()
* Sets up the serial port EUSCI_A0 as 9600 8N1 (8 bits, no parity,
* one stop bit.) Enables the interrupt so that received data will
